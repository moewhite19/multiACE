...
Since the heavy work runs on the Snapmaker (limited RAM/CPU), you can run the exact same logic on your PC instead — it's the standalone version of the engine. It does two things: it rewrites the G-code in place (injects the ACE_SWAP_HEAD commands) and writes a report with the loadout/swap plan.
