...
createApp({
  setup() {
    ...
    const tab = ref(_validTabs.has(_storedTab) ? _storedTab : "dashboard");
    ...
    function _makePerAceEntry() {
      ...
      return {
        ...
        fa_print_disable: false, fa_load_disable: false,
    ...
    function paramsToForm(params, perAceParams) {
      ...
      const parseIdxList = (v) => new Set(
        String(v || '').split(',').map(s => s.trim()).filter(Boolean).map(Number));
      const printSet = parseIdxList(params.fa_print_disable);
      const loadSet  = parseIdxList(params.fa_load_disable);
      ...
      for (let i = 0; i < configForm.perAce.length; i++) {
        ...
        configForm.perAce[i].fa_print_disable = printSet.has(i);
        configForm.perAce[i].fa_load_disable  = loadSet.has(i);
    ...
    function formToCfgContent(content) {
      ...
      const printDisable = configForm.perAce
        .map((p, i) => p.fa_print_disable ? i : -1)
        .filter(i => i >= 0);
      const loadDisable = configForm.perAce
        .map((p, i) => p.fa_load_disable ? i : -1)
        .filter(i => i >= 0);
      ...
      const mainRepl = {
        ...
        fa_print_disable:   printDisable.length ? printDisable.join(',') : '',
        fa_load_disable:    loadDisable.length  ? loadDisable.join(',')  : '',
      };
      for (let i = 0; i < configForm.perAce.length; i++) {
    ...
    function popoutDragStart(ev) {
      // Only the header bar is the drag handle; ignore clicks on the
      // close button (which has its own handler with stopPropagation).
    ...
    function onUploadGcode(fileList) {
      ...
      confirm({
        title: t("ui.upload.confirm_title"),
        message: t("ui.upload.confirm_msg", {name: f.name, size: sizeMb}),
        okLabel: t("ui.upload.confirm_ok"),
        onOk: async () => {
          uploading.value = true;
          try {
            const fd = new FormData();
            fd.append("file", f, f.name);
            const r = await fetch(`${API}/upload-and-print`, {method: "POST", body: fd});
            if (!r.ok) {
              let msg = `${r.status} ${r.statusText}`;
              try { const j = await r.json(); if (j.detail) msg = j.detail; } catch (_) {}
              throw new Error(msg);
            }
            setMacroLog(t("ui.upload.started", {name: f.name}));
          } catch (e) {
            setMacroLog(`${t("ui.upload.failed")}: ${e.message || e}`);
            confirm({
              title: t("ui.upload.title"),
              message: `${t("ui.upload.failed")}: ${e.message || e}`,
              dismissOnly: true, okLabel: "OK", onOk: () => {},
            });
          } finally {
    ...
    onMounted(async () => {
      ...
      try { const r = await fetch(`${API}/version`); if (r.ok) version.value = `v${(await r.json()).web}`; } catch (_) {}
      try { const r = await fetch(`${API}/screen-available`); if (r.ok) screenAvailable.value = (await r.json()).available; } catch (_) {}
      await reloadState();
      await reloadSnapshots();
      await loadConfig();
      await loadNotifications();
      // if Dashboard is selected but mode=normal, redirect to Config
      if (state.mode === "normal" && tab.value === "dashboard") tab.value = "config";
      wsConnect();
      // Observe container size for wiring re-layout
    ...
    function _onBeforeUnload(ev) {
      const pending = cmdQueue.value.some(
        it => it.status === 'queued' || it.status === 'running');
      if (!pending) return;
      // Modern browsers ignore the message text and show their own
      // generic 'leave this page?' prompt — preventDefault + a
      // truthy returnValue is enough to trigger it.
      ev.preventDefault();
      ev.returnValue = '';
      return '';
    }
    ...
    return {
      tab, version, connClass, connText, screenAvailable,
