...
class FeedLight:
    ...
    def __init__(self, printer, reactor, red_pin, white_pin):
        ...
        self.red_light.setup_max_duration(0.0)
        ...
        self.white_light.setup_max_duration(0.0)
    ...
    def set_light_state(self, print_time, state, index=None, value=None):
        if state in [FEED_STA_PRELOAD_PREPARE, FEED_STA_LOAD_PREPARE, FEED_STA_UNLOAD_PREPARE, FEED_STA_MANUAL_PREPARE]:
            self.red_light.set_pwm(print_time, 0)
            self.white_light.set_pwm(print_time, 0.2)
        elif state in [FEED_STA_PRELOAD_FEEDING, FEED_STA_LOAD_HOMING, FEED_STA_LOAD_PICKING, FEED_STA_LOAD_HEATING, FEED_STA_LOAD_FEEDING, FEED_STA_LOAD_EXTRUDING, FEED_STA_LOAD_FLUSHING, FEED_STA_UNLOAD_HOMING, FEED_STA_UNLOAD_PICKING, FEED_STA_UNLOAD_HEAT_FINISH, FEED_STA_UNLOAD_HEATING, FEED_STA_UNLOAD_DOING, FEED_STA_MANUAL_HOMING, FEED_STA_MANUAL_PICKING, FEED_STA_MANUAL_PREPARE_FINISH, FEED_STA_MANUAL_HEATING, FEED_STA_MANUAL_EXTRUDING, FEED_STA_MANUAL_EXTRUDE_FINISH, FEED_STA_MANUAL_FLUSHING, FEED_STA_MANUAL_FLUSH_FINISH]:
            self.red_light.set_pwm(print_time, 0)
            self.white_light.set_pwm(print_time, 0.5)
        elif state in [FEED_STA_PRELOAD_FINISH, FEED_STA_LOAD_FINISH, FEED_STA_UNLOAD_FINISH, FEED_STA_MANUAL_FINISH]:
            self.red_light.set_pwm(print_time, 0)
            self.white_light.set_pwm(print_time, 1)
        elif state in [FEED_STA_PRELOAD_FAIL, FEED_STA_LOAD_FAIL, FEED_STA_UNLOAD_FAIL, FEED_STA_MANUAL_PREPARE_FAIL, FEED_STA_MANUAL_EXTRUDE_FAIL, FEED_STA_MANUAL_FLUSH_FAIL, FEED_STA_MANUAL_FAIL]:
...
class FeedTachometer:
    ...
    def get_rpm(self):
        rpm = self.frequence.get_frequency() * 30.0 / self.ppr
        return rpm
...
class FilamentFeed:
    ...
    def __init__(self, config):
        ...
        tmp_obj = FeedLight(self.printer, self.reactor, white_pin, red_pin)
        self.light.append(tmp_obj)
        self.gcode.register_mux_command('FEED_LIGHT', 'MODULE', self.module_name, self.cmd_FEED_LIGHT)
        self._port = []
        ...
        self._port.append(tmp_obj)
        self.gcode.register_mux_command('FEED_PORT', 'MODULE', self.module_name, self.cmd_FEED_PORT)
        self.wheel = []
        self.wheel_2 = []
        tmp_pin = config.get('wheel_tach_ch_1_1_pin')
        wheel_tach_ppr = config.getint('wheel_tach_ppr', 6, minval=1)
        poll_time = config.getfloat('wheel_tach_poll_interval', 0.0005, above=0.0)
        tmp_obj = FeedTachometer(self.printer, tmp_pin, wheel_tach_ppr, 0.1, poll_time)
        ...
        tmp_obj = FeedTachometer(self.printer, tmp_pin, wheel_tach_ppr, 0.1, poll_time)
        self.wheel_2.append(tmp_obj)
        self.gcode.register_mux_command('FEED_WHEEL_TACH', 'MODULE', self.module_name, self.cmd_FEED_WHEEL_TACH)
        ...
        self.gcode.register_mux_command('FEED_MOTOR', 'MODULE', self.module_name, self.cmd_FEED_MOTOR)
        self.gcode.register_mux_command('FEED_MOTOR_ONE_CYCLE', 'MODULE', self.module_name, self.cmd_FEED_MOTOR_ONE_CYCLE)
        tmp_pin = config.get('motor_tach_pin')
        motor_tach_ppr = config.getint('motor_tach_ppr', 2, minval=1)
        poll_time = config.getfloat('motor_tach_poll_interval', 0.0015, above=0.0)
        self.motor_tachometer = FeedTachometer(self.printer, tmp_pin, motor_tach_ppr, 0.1, poll_time)
        self.gcode.register_mux_command('FEED_MOTOR_TACH', 'MODULE', self.module_name, self.cmd_FEED_MOTOR_TACH)
        ...
        self.gcode.register_mux_command('FEED_AUTO', 'MODULE', self.module_name, self.cmd_FEED_AUTO)
        self.gcode.register_mux_command('FEED_MANUAL', 'MODULE', self.module_name, self.cmd_FEED_MANUAL)
        self.gcode.register_mux_command('FEED_RUNOUT_EVENT_HANDLE', 'MODULE', self.module_name, self.cmd_FEED_RUNOUT_EVENT_HANDLE)
    ...
    def _ready(self):
        ...
        self.reactor.update_timer(self._check_init_state_timer, self.reactor.monotonic() + 2 * FEED_PORT_ADC_REPORT_TIME)
    ...
    def _runout_evt_handle(self, extruder, present):
        ...
        for ch in range(FEED_CHANNEL_NUMS):
            if extruder == self.filament_ch[ch]:
                self.reactor.register_async_callback(lambda et, c=self._do_feed, ch=ch, action=FEED_ACT_FILAMENT_RUNOUT: c(ch, action))
                break
        ...
        for ch in range(FEED_CHANNEL_NUMS):
            ...
            if self.config['auto_mode'][ch] == True and self.module_exist[ch] == True:
                if self.filament_detect.is_startup_stay() == False:
                    self.printer.send_event('filament_feed:port', self.filament_ch[ch], self._port[ch].get_filament_detected())
                if self._port[ch].get_filament_detected() == False:
                    self._set_channel_state(ch, FEED_STA_WAIT_INSERT)
                elif self.config['load_finish'][ch] == True:
    ...
    def _set_channel_state(self, channel, state, save=False):
        ...
        if state not in [FEED_STA_INITED, FEED_STA_WAIT_INSERT, FEED_STA_TEST] and (not state.startswith('preload_')):
    ...
    def _port_event_handler(self, detected, channel):
        if self.config['auto_mode'][channel] == False or self.module_exist[channel] == False:
            return
        ...
        if detected:
            if self.channel_state[channel] == FEED_STA_PRELOAD_PREPARE:
                ...
                self.reactor.register_async_callback(lambda et, c=self._do_feed, ch=channel, action=FEED_ACT_PRELOAD: c(ch, action))
        else:
            if self.channel_active != channel:
                self._set_light_state(channel, FEED_STA_WAIT_INSERT)
            self.reactor.register_async_callback(lambda et, c=self._do_feed, ch=channel, action=FEED_ACT_REMOVE_FILAMENT: c(ch, action))
    ...
    def _get_filament_temp(self, channel):
        ...
        return filament_parameters.get_load_temp(status['filament_vendor'][self.filament_ch[channel]], status['filament_type'][self.filament_ch[channel]], status['filament_sub_type'][self.filament_ch[channel]])
    ...
    def _get_filament_soft(self, channel):
        ...
        if print_task_config is None or filament_parameters is None:
            return False
        status = print_task_config.get_status()
        return filament_parameters.get_is_soft(status['filament_vendor'][self.filament_ch[channel]], status['filament_type'][self.filament_ch[channel]], status['filament_sub_type'][self.filament_ch[channel]])
    ...
    def _hang_neutral(self, channel):
        ...
        for retry in range(2):
            if channel == FEED_CHANNEL_1:
                self.motor.run_one_cycle(FEED_MOTOR_DIR_B, self.motor_speed_hang_neutral_b, self.motor_hang_neutral_time)
            else:
                self.motor.run_one_cycle(FEED_MOTOR_DIR_A, self.motor_speed_hang_neutral_a, self.motor_hang_neutral_time)
            self.reactor.pause(self.reactor.monotonic() + 0.105)
    ...
    def _is_keep_raw_error_info(self, error=None):
        if error in [FEED_ERR_MOVE, FEED_ERR_MOVE_HOME, FEED_ERR_MOVE_SWITCH, FEED_ERR_HEAT]:
            return True
        else:
            return False
    ...
    def _do_feed(self, ch, action=None, stage=None, auto_mode=None):
        ...
        if action == FEED_ACT_UPDATE_AUTO_MODE and auto_mode is None:
            logging.error('[feed] parameter error!')
            return
        if action in [FEED_ACT_PRELOAD, FEED_ACT_LOAD] and (self.config['auto_mode'][ch] == False or self.module_exist[ch] == False):
            return
        wheel_cnt_a_1 = 0
        wheel_cnt_b_1 = 0
        motor_cnt_1 = 0
        ...
        try:
            if action == FEED_ACT_UPDATE_AUTO_MODE:
                ...
                try:
                    ...
                    motor_cnt_1 = self.motor_tachometer.get_counts()
                    logging.info('[feed_preload] extruder[%d], start, wheel_cnt_a: %d, wheel_cnt_b: %d, motor_cnt: %d', self.filament_ch[ch], wheel_cnt_a_1, wheel_cnt_b_1, motor_cnt_1)
                    ...
                    if arrive_runout_sensor == False:
                        while 1:
                            ...
                            if port_detect == False:
                                ...
                                self.exception_code[ch] = 13
                                break
                            if (wheel_cnt_a_2 - wheel_cnt_a_1) / self.wheel[ch].ppr > self._feed_preload_counts or (wheel_cnt_b_2 - wheel_cnt_b_1) / self.wheel_2[ch].ppr > self._feed_preload_counts:
                                self.channel_error[ch] = FEED_OK
                                break
                            if motor_speed < FEED_PRELOAD_MOTOR_MIN_SPEED:
                                ...
                                motor_speed_err_max = FEED_PRELOAD_MOTOR_ERR_CNT_MAX
                            if wheel_speed_a * FEED_MOTOR_REDUCTION_R < motor_speed * (1 - FEED_MOTOR_SLIP_RATE) and wheel_speed_b * FEED_MOTOR_REDUCTION_R < motor_speed * (1 - FEED_MOTOR_SLIP_RATE):
                    ...
                    wheel_cnt_b_2 = self.wheel_2[ch].get_counts()
                    motor_cnt_2 = self.motor_tachometer.get_counts()
                    logging.info('[feed_preloading] extruder[%d], wheel, cnt_a_1:%d, cnt_b_1:%d, cnt_a_2:%d, cnt_b_2:%d, wheel_speed_a:%d, wheel_speed_b: %d, motor, motor_cnt_1:%d, motor_cnt_2:%d, motor_speed:%d', self.filament_ch[ch], wheel_cnt_a_1, wheel_cnt_b_1, wheel_cnt_a_2, wheel_cnt_b_2, wheel_speed_a, wheel_speed_b, motor_cnt_1, motor_cnt_2, motor_speed)
                    ...
                    if self.exception_manager is not None:
                        self.exception_manager.raise_exception_async(id=self.exception_manager.list.MODULE_ID_FEEDING, index=self.filament_ch[ch], code=self.exception_code[ch], message='preload fail: %s' % self.channel_error[ch], oneshot=1, level=1)
                finally:
                ...
                if self.ace is not None:
                    self.ace._fa_trace('FEED_ACT_LOAD enter: ch=%d head=%d active_ace=%d' % (ch, self.filament_ch[ch], self.ace._active_device_index))
                ...
                try:
                    ...
                    if self.runout_sensor[ch].get_status(0)['filament_detected'] == False:
                        ...
                        if self.ace is not None:
                            ...
                            self.ace._fa_trace('feed phase enter: ace=%d ch=%d head=%d' % (ace_idx, ch, self.filament_ch[ch]))
                            if ace_idx in self.ace._fa_load_disable:
                                logging.info('[multiACE] FEED_AUTO LOAD: ACE %d in fa_load_disable, skipping feed+FA' % ace_idx)
                                self.channel_error[ch] = FEED_OK
                            else:
                                ...
                                for load_attempt in range(load_retries + 1):
                                    if load_attempt > 0:
                                        logging.info('[feed_loading] retry %d/%d: retracting %dmm', load_attempt, load_retries, load_retry_retract)
                                    ...
                                    while 1:
                                        ...
                                        if runout_detect == True:
                                            ...
                                            overshoot = getattr(self.ace, 'seat_overshoot_length', 0)
                                            if overshoot > 0:
                                                self.ace.wait_ace_ready()
                                                self.ace._feed(self.filament_ch[ch], overshoot, self.ace.feed_speed)
                                                self.ace.wait_ace_ready()
                                                logging.info('[feed_loading] post-sensor overshoot %dmm @%dmm/s done', overshoot, self.ace.feed_speed)
                                            self.channel_error[ch] = FEED_OK
                                            load_found = True
                                            break
                    ...
                    if self.ace is not None and self.ace._active_device_index not in self.ace._fa_load_disable:
                        ...
                        logging.info('[multiACE] FEED_AUTO LOAD: about to call _arm_fa_for idx=%d slot=%d auto_feed=%s fa_context=%s' % (self.ace._active_device_index, head_idx, self.ace._auto_feed_enabled, self.ace._fa_context))
                        try:
                            self.ace._arm_fa_for(self.ace._active_device_index, head_idx)
                        except Exception as fa_e:
                            logging.info('[multiACE] FEED_AUTO LOAD: _arm_fa_for failed: %s' % fa_e)
                    ...
                    try:
                        ...
                        for retry in range(extrude_max):
                            ...
                            for retry_extrude in range(retry_extrude_times):
                                ...
                                wheel_cnt_a_2 = self.wheel[ch].get_counts()
                                wheel_cnt_b_2 = self.wheel_2[ch].get_counts()
                                logging.info('[feed_loading] phase3: extrude[%d] retry:%d, retry_extrude:%d, coil_freq_start:%d, coil_freq_end_min:%d, coil_freq_end_max:%d, coil_freq_delta:%d', self.filament_ch[ch], retry, retry_extrude, coil_freq_start, coil_freq_end_min, coil_freq_end_max, coil_freq_end_max - coil_freq_end_min)
                                logging.info('[feed_loading] phase3: wheel, cnt_a_1:%d, cnt_a_2:%d, cnt_b_1:%d, cnt_b_2:%d', wheel_cnt_a_1, wheel_cnt_a_2, wheel_cnt_b_1, wheel_cnt_b_2)
                                _wlog_push = getattr(self.ace, '_wiggle_log', None) if self.ace is not None else None
                                if _wlog_push is not None:
                                    _wlog_push.info('phase3 head=%d ace=%s slot=%s retry=%d r_e=%d coil_start=%d coil_min=%d coil_max=%d coil_delta=%d wheel_a=%d wheel_b=%d', self.filament_ch[ch], ace_idx_p3, slot_p3, retry, retry_extrude, coil_freq_start, coil_freq_end_min, coil_freq_end_max, coil_freq_end_max - coil_freq_end_min, wheel_cnt_a_2 - wheel_cnt_a_1, wheel_cnt_b_2 - wheel_cnt_b_1)
                                step_delta_a = wheel_cnt_a_2 - prev_a_p3
                                step_delta_b = wheel_cnt_b_2 - prev_b_p3
                                coil_high = abs(coil_freq_end_min - coil_freq_start) >= 5000 or abs(coil_freq_end_max - coil_freq_start) >= 5000
                                step_ok = retry_extrude == 0 or step_delta_a >= 2 or step_delta_b >= 2 or coil_high
                                if self.check_wheel_data != 0 and self.check_coil_freq == 0:
                                    if (wheel_cnt_a_2 - wheel_cnt_a_1 >= 5 or wheel_cnt_b_2 - wheel_cnt_b_1 >= 5) and step_ok:
                                        extruded = True
                                        break
                                elif self.check_wheel_data == 0 and self.check_coil_freq != 0:
                                    if retry > 0 and inductance_coil is not None:
                                        if abs(coil_freq_end_min - coil_freq_start) >= coil_freq_threshold or abs(coil_freq_end_max - coil_freq_start) >= coil_freq_threshold:
                                            extruded = True
                                            break
                                else:
                                    wheel_ok = wheel_cnt_a_2 - wheel_cnt_a_1 >= 5 or wheel_cnt_b_2 - wheel_cnt_b_1 >= 5
                                    coil_ok = True
                                    if retry > 0 and inductance_coil is not None:
                                        coil_ok = abs(coil_freq_end_min - coil_freq_start) >= coil_freq_threshold or abs(coil_freq_end_max - coil_freq_start) >= coil_freq_threshold
                            ...
                            if extruded == True:
                                _wlog = getattr(self.ace, '_wiggle_log', None) if self.ace else None
                                if _wlog is not None:
                                    _wlog.info('phase3 head=%d ace=%s slot=%s SUCCESS at retry=%d wiggles_used=%s coil_start=%d coil_min=%d coil_max=%d coil_delta=%d', self.filament_ch[ch], ace_idx_p3, slot_p3, retry, phase3_wiggles_used or '(none)', coil_freq_start, coil_freq_end_min, coil_freq_end_max, coil_freq_end_max - coil_freq_end_min)
                                break
                            if retry == 0:
                                logging.info('[feed_loading] phase3: retry:0 — skip cleanup, advance to retry 1')
                                continue
                            scheme = getattr(self.ace, 'wiggle_scheme', 'EEEEE') if self.ace is not None else 'EEEEE'
                            wiggle_idx = retry - 1
                            ...
                            if wiggle_log is not None:
                                wiggle_log.info('phase3 head=%d ace=%s slot=%s scheme=%s retry=%d type=%s retract=%d push=%d START', head_for_log, ace_idx_p3, slot_p3, scheme, retry, wiggle_char, retract_mm, push_mm)
                            if self.ace is not None and ace_idx_p3 is not None and (slot_p3 is not None):
                                try:

                                    def _phase3_stop_cb(self, response):
                                        pass
                                    self.ace.send_request_to(ace_idx_p3, {'method': 'stop_feed_assist', 'params': {'index': slot_p3}}, _phase3_stop_cb)
                                    self.ace._feed_assist_per_ace[ace_idx_p3] = -1
                                    if ace_idx_p3 == self.ace._active_device_index:
                                        self.ace._feed_assist_index = -1
                                    self.ace.wait_ace_ready()
                                    self.ace._fa_trace('phase3 wiggle %s: stop FA slot=%d on ACE %d' % (wiggle_char, slot_p3, ace_idx_p3))
                                except Exception as e:
                                    logging.info('[multiACE] phase3 stop_feed_assist failed: %s' % e)
                            if wiggle_char == 'A':
                                if self.ace is not None and ace_idx_p3 is not None and (slot_p3 is not None):
                                    head_idx_for_ext = self.filament_ch[ch]
                                    extruder_name = 'extruder' if head_idx_for_ext == 0 else 'extruder%d' % head_idx_for_ext
                                    extruder_disabled = False
                                    try:
                                        self.gcode.run_script_from_command('SET_STEPPER_ENABLE STEPPER=%s ENABLE=0\r\n' % extruder_name)
                                        self.toolhead.wait_moves()
                                        extruder_disabled = True
                                        self.ace._fa_trace('phase3 A wiggle: disabled %s for free-pull' % extruder_name)
                                    except Exception as e:
                                        logging.info('[multiACE] phase3 extruder disable failed: %s' % e)
                                    try:
                                        ...
                                        def _phase3_a_cb(self, response):
                                            pass
                                        self.ace.send_request_to(ace_idx_p3, {'method': 'unwind_filament', 'params': {'index': slot_p3, 'length': retract_mm, 'speed': retract_speed}}, _phase3_a_cb)
                                        self.reactor.pause(self.reactor.monotonic() + retract_mm / max(1, retract_speed) + 0.5)
                                        self.ace.wait_ace_ready()
                                        self.ace.send_request_to(ace_idx_p3, {'method': 'feed_filament', 'params': {'index': slot_p3, 'length': push_mm, 'speed': feed_speed}}, _phase3_a_cb)
                                        self.reactor.pause(self.reactor.monotonic() + push_mm / max(1, feed_speed) + 0.5)
                                        self.ace.wait_ace_ready()
                                        self.ace._fa_trace('phase3 ACE wiggle done slot=%d on ACE %d (retract=%d push=%d)' % (slot_p3, ace_idx_p3, retract_mm, push_mm))
                                    except Exception as e:
                                        logging.info('[multiACE] phase3 ACE wiggle failed: %s' % e)
                                    finally:
                                        if extruder_disabled:
                                            try:
                                                self.gcode.run_script_from_command('SET_STEPPER_ENABLE STEPPER=%s ENABLE=1\r\n' % extruder_name)
                                                self.toolhead.wait_moves()
                                                self.ace._fa_trace('phase3 A wiggle: re-enabled %s' % extruder_name)
                                            except Exception as e:
                                                logging.info('[multiACE] phase3 extruder re-enable failed: %s' % e)
                            else:
                                self.gcode.run_script_from_command('G1 E-%d F600\r\n' % retract_mm)
                                self.toolhead.wait_moves()
                                self.reactor.pause(self.reactor.monotonic() + 0.2)
                                if self.ace is not None and ace_idx_p3 is not None and (slot_p3 is not None):
                                    try:
                                        self.ace._arm_fa_for(ace_idx_p3, slot_p3)
                                        self.ace._fa_trace('phase3 E wiggle: restart FA slot=%d on ACE %d before push' % (slot_p3, ace_idx_p3))
                                    except Exception as e:
                                        logging.info('[multiACE] phase3 FA restart failed: %s' % e)
                                if filament_soft:
                                    self.gcode.run_script_from_command('G1 E%d F200\r\n' % push_mm)
                                else:
                                    self.gcode.run_script_from_command('G1 E%d F480\r\n' % push_mm)
                            ...
                            if wiggle_log is not None:
                                wiggle_log.info('phase3 head=%d ace=%s slot=%s retry=%d type=%s DONE', head_for_log, ace_idx_p3, slot_p3, retry, wiggle_char)
                    except Exception as e:
                        ...
                        _wlog = getattr(self.ace, '_wiggle_log', None) if self.ace else None
                        if _wlog is not None:
                            _wlog.info('phase3 head=%d ace=%s slot=%s FAILED (exception) wiggles_used=%s err=%s', self.filament_ch[ch], ace_idx_p3, slot_p3, phase3_wiggles_used or '(none)', str(e))
                    ...
                    if extruded == False:
                        ...
                        _wlog = getattr(self.ace, '_wiggle_log', None) if self.ace else None
                        if _wlog is not None:
                            _wlog.info('phase3 head=%d ace=%s slot=%s FAILED (no movement) wiggles_used=%s last_coil_delta=%d', self.filament_ch[ch], ace_idx_p3, slot_p3, phase3_wiggles_used or '(none)', coil_freq_end_max - coil_freq_end_min)
                        self._snapshot_inner_resume_state()
                        raise ValueError('logic error!')
                    ...
                    if extra_purge > 0:
                        try:
                            self.gcode.run_script_from_command('G1 E%.1f F300\r\n' % extra_purge)
                            self.toolhead.wait_moves()
                        except Exception as e:
                    ...
                    if self.ace is not None and getattr(self.ace, '_ace_mode', '') == 'multi':
                        ...
                        if ace_slot < len(slot_info):
                            ...
                            si = {}
                        self.ace._head_source[head_idx] = {'ace_index': ace_idx, 'slot': ace_slot, 'type': si.get('type', 'PLA'), 'color': self.ace.rgb2hex(*si.get('color', (0, 0, 0))), 'brand': si.get('brand', 'Generic')}
                        self.ace._save_head_source()
                        self.ace._ghost_heads.discard(head_idx)
                        logging.info('[multiACE] FEED_AUTO LOAD: head_source[%d] -> ACE %d / Slot %d' % (head_idx, ace_idx, ace_slot))
                    ...
                    if fa_gate_opened and self.ace is not None:
                        ...
                        try:
                            ...
                            logging.info('[multiACE] FA disable after load failed: %s' % fa_e)
            elif action == FEED_ACT_UNLOAD:
                if self.ace is not None:
                    self.ace._disable_feed_assist_all()
                self.exception_code[ch] = 70
                if stage not in [None, FEED_UNLOAD_STAGE_PREPARE, FEED_UNLOAD_STAGE_DOING, FEED_UNLOAD_STAGE_CANCEL]:
                ...
                if stage == FEED_UNLOAD_STAGE_PREPARE:
                    ...
                    try:
                        _stable_states = [FEED_STA_UNLOAD_HEAT_FINISH, FEED_STA_UNLOAD_FINISH, FEED_STA_LOAD_FINISH, FEED_STA_LOAD_FAIL, FEED_STA_PRELOAD_FINISH, FEED_STA_PRELOAD_FAIL, FEED_STA_NONE, FEED_STA_INITED, FEED_STA_WAIT_INSERT]
                        ...
                        self.gcode.run_script_from_command('SET_FILAMENT_SENSOR SENSOR=%s ENABLE=1\r\n' % sensor_name)
                        try:
                        ...
                        if self.ace is not None:
                            ...
                            for unload_attempt in range(unload_max):
                                ...
                                if not self.runout_sensor[ch].get_status(0)['filament_detected']:
                                    logging.info('[feed][unload] sensor cleared (attempt %d/%d)', unload_attempt + 1, unload_max)
                                    unload_ok = True
                                    break
                                logging.info('[feed][unload] sensor still detected, retry %d/%d - re-heat + re-unload', unload_attempt + 1, unload_max)
                                try:
                                    self.gcode.run_script_from_command('M109 S%d\r\n' % filament_feed_temp)
                                    self.toolhead.wait_moves()
                                    self.gcode.run_script_from_command('INNER_FILAMENT_UNLOAD TEMP=%d SOFT=%d\r\n' % (filament_feed_temp, int(filament_soft)))
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.channel_error[ch] = FEED_ERR_HEAT
                            raise
                        self.gcode.run_script_from_command('SET_FILAMENT_SENSOR SENSOR=e%d_filament ENABLE=1\r\n' % self.filament_ch[ch])
                        try:
                            self._set_channel_state(ch, FEED_STA_UNLOAD_DOING)
                            self.toolhead.wait_moves()
                            self.gcode.run_script_from_command('INNER_FILAMENT_UNLOAD TEMP=%d SOFT=%d\r\n' % (filament_feed_temp, int(filament_soft)))
                ...
                if stage not in [FEED_MANUAL_STAGE_PREPARE, FEED_MANUAL_STAGE_EXTRUDE, FEED_MANUAL_STAGE_FLUSH, FEED_MANUAL_STAGE_FINISH, FEED_MANUAL_STAGE_CANCEL]:
                ...
                if stage == FEED_MANUAL_STAGE_PREPARE:
                    ...
                    if self.module_exist[ch] == True and self.config['auto_mode'][ch] == True:
                        if self._port[ch].get_filament_detected() == False:
                            self._set_channel_state(ch, FEED_STA_WAIT_INSERT)
                        elif self.runout_sensor[ch] is not None and self.runout_sensor[ch].get_status(0)['enabled'] == True and (self.runout_sensor[ch].get_status(0)['filament_detected'] == True):
                            self._set_channel_state(ch, FEED_STA_LOAD_FINISH, True)
                        else:
                            self._set_channel_state(ch, FEED_STA_PRELOAD_FINISH)
    ...
    def get_status(self, eventtime=None):
        ...
        def _runout(ch):
            ...
            try:
                ...
                return None
        in_ace_1 = self.ace.gate_status[self.filament_ch[FEED_CHANNEL_1]] == 1 if self._port[FEED_CHANNEL_1].ace is not None else None
        in_ace_2 = self.ace.gate_status[self.filament_ch[FEED_CHANNEL_2]] == 1 if self._port[FEED_CHANNEL_2].ace is not None else None
        channel_1_dist = {'module_exist': self.module_exist[FEED_CHANNEL_1], 'filament_detected': filament_detected[FEED_CHANNEL_1], 'filament_in_ace': in_ace_1, 'filament_in_toolhead': self._port[FEED_CHANNEL_1].get_filament_detected_local(), 'filament_at_extruder': _runout(FEED_CHANNEL_1), 'disable_auto': not self.config['auto_mode'][FEED_CHANNEL_1], 'channel_state': self.channel_state[FEED_CHANNEL_1], 'channel_error': self.channel_error[FEED_CHANNEL_1], 'channel_error_state': self.channel_error_state[FEED_CHANNEL_1], 'channel_action_state': self.channel_action_state[FEED_CHANNEL_1]}
        channel_2_dist = {'module_exist': self.module_exist[FEED_CHANNEL_2], 'filament_detected': filament_detected[FEED_CHANNEL_2], 'filament_in_ace': in_ace_2, 'filament_in_toolhead': self._port[FEED_CHANNEL_2].get_filament_detected_local(), 'filament_at_extruder': _runout(FEED_CHANNEL_2), 'disable_auto': not self.config['auto_mode'][FEED_CHANNEL_2], 'channel_state': self.channel_state[FEED_CHANNEL_2], 'channel_error': self.channel_error[FEED_CHANNEL_2], 'channel_error_state': self.channel_error_state[FEED_CHANNEL_2], 'channel_action_state': self.channel_action_state[FEED_CHANNEL_2]}
        return {f'extruder{self.filament_ch[FEED_CHANNEL_1]}': channel_1_dist, f'extruder{self.filament_ch[FEED_CHANNEL_2]}': channel_2_dist}
    ...
    def cmd_FEED_WHEEL_TACH(self, gcmd):
        ...
        if channel < 0 or channel >= FEED_CHANNEL_NUMS:
            raise gcmd.error('[feed] channel[%d] is out of range[0,%d]\n' % (channel, FEED_CHANNEL_NUMS - 1))
        msg = 'rpm: %d\ncnt: %d\nrpm2: %d\ncnt2: %d\n' % (self.wheel[channel].get_rpm(), self.wheel[channel].get_counts(), self.wheel_2[channel].get_rpm(), self.wheel_2[channel].get_counts())
        gcmd.respond_info(msg, log=False)
    ...
    def cmd_FEED_MOTOR_TACH(self, gcmd):
        msg = 'rpm: %d\ncnt: %d\n' % (self.motor_tachometer.get_rpm(), self.motor_tachometer.get_counts())
        gcmd.respond_info(msg, log=False)
    ...
    def cmd_FEED_AUTO(self, gcmd):
        ...
        filament_entangle_detect = self.printer.lookup_object(f'filament_entangle_detect e{self.filament_ch[channel]}_filament', None)
        ...
        if auto_mode is not None:
            try:
                ...
                raise gcmd.error(message='[feed] channel[%d]: set auto mode error \n' % channel, action='none', id=525, index=self.filament_ch[channel], code=0, oneshot=1, level=2)
            if need_save:
        ...
        if need_to_load == True:
            ...
            if self.channel_state[channel] != FEED_STA_LOAD_FINISH or self.channel_error[channel] != FEED_OK:
                ...
                tech_msg = 'extruder[%d]: state: %s, error: %s!' % (self.filament_ch[channel], self.channel_error_state[channel], self.channel_error[channel])
                if raw_msg is not None:
                    tech_msg = tech_msg + 'raw msg:' + raw_msg
                if self.ace is not None and getattr(self.ace, '_ace_mode', '') == 'multi':
                    head_idx = self.filament_ch[channel]
                    short = 'T%d load failed' % head_idx
                    hint = 'Filament jam on T%d — reload via display, then RESUME (see fluidd log)' % head_idx
                    try:
                        self.gcode.run_script_from_command('M117 %s' % short)
                    except Exception:
                        pass
                    try:
                        self.gcode.run_script_from_command('RESPOND TYPE=error MSG="[multiACE] PAUSE T%d: %s"' % (head_idx, hint.replace('"', "'")))
                    except Exception:
                        pass
                    for step in ('Open display load menu, reload T%d filament' % head_idx, 'Verify filament is in the toolhead', 'Press RESUME on display or in fluidd to continue'):
                        try:
                            self.gcode.run_script_from_command('RESPOND TYPE=echo MSG="  - %s"' % step)
                        except Exception:
                            pass
                    try:
                        self.ace._audit_state('PAUSE_FEED_LOAD_FAIL', {'channel': channel, 'head': head_idx, 'tech_msg': tech_msg})
                    except Exception:
                        pass
                    msg = '%s (%s)' % (hint, tech_msg)
                else:
                    msg = tech_msg
                raise gcmd.error(message=msg, action='pause', id=525, index=self.filament_ch[channel], code=self.exception_code[channel], oneshot=1, level=2)
            return
        if need_to_unload == True:
            ...
            if self.channel_error[channel] != FEED_OK:
                msg = 'extruder[%d]: state: %s, error: %s!' % (self.filament_ch[channel], self.channel_error_state[channel], self.channel_error[channel])
                if raw_msg is not None:
