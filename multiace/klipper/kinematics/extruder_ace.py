...
class ExtruderSwitchRecorder:
    ...
    def _init_extruder_entry(self, extruder_name):
        if extruder_name not in self.data:
            self.data[extruder_name] = {'switch_count': 0, 'retry_count': 0, 'error_count': 0, 'last_maintenance_count': 0}
        else:
...
class ExtruderStepper:
    ...
    def __init__(self, config):
        ...
        self.config_smooth_time = config.getfloat('pressure_advance_smooth_time', 0.04, above=0.0, maxval=0.2)
        ...
        if self.name == 'extruder':
            gcode.register_mux_command('SET_PRESSURE_ADVANCE', 'EXTRUDER', None, self.cmd_default_SET_PRESSURE_ADVANCE, desc=self.cmd_SET_PRESSURE_ADVANCE_help)
        gcode.register_mux_command('SET_PRESSURE_ADVANCE', 'EXTRUDER', self.name, self.cmd_SET_PRESSURE_ADVANCE, desc=self.cmd_SET_PRESSURE_ADVANCE_help)
        gcode.register_mux_command('SET_EXTRUDER_ROTATION_DISTANCE', 'EXTRUDER', self.name, self.cmd_SET_E_ROTATION_DISTANCE, desc=self.cmd_SET_E_ROTATION_DISTANCE_help)
        gcode.register_mux_command('SYNC_EXTRUDER_MOTION', 'EXTRUDER', self.name, self.cmd_SYNC_EXTRUDER_MOTION, desc=self.cmd_SYNC_EXTRUDER_MOTION_help)
    ...
    def get_status(self, eventtime):
        return {'pressure_advance': self.pressure_advance, 'smooth_time': self.pressure_advance_smooth_time, 'motion_queue': self.motion_queue}
    ...
    def _set_pressure_advance(self, pressure_advance, smooth_time):
        ...
        if new_smooth_time != old_smooth_time:
            toolhead.note_step_generation_scan_time(new_smooth_time * 0.5, old_delay=old_smooth_time * 0.5)
        ffi_main, ffi_lib = chelper.get_ffi()
    ...
    def cmd_SET_PRESSURE_ADVANCE(self, gcmd):
        ...
        if print_config is not None and print_stats is not None and (print_config.print_task_config['flow_calibrate'] == True):
        ...
        smooth_time = gcmd.get_float('SMOOTH_TIME', self.pressure_advance_smooth_time, minval=0.0, maxval=0.2)
        old_pressure_advance = self.pressure_advance
        old_smooth_time = self.pressure_advance_smooth_time
        self._set_pressure_advance(pressure_advance, smooth_time)
        virtual_sdcard = self.printer.lookup_object('virtual_sdcard', None)
        if virtual_sdcard is not None and (old_pressure_advance != self.pressure_advance or old_smooth_time != self.pressure_advance_smooth_time):
    ...
    def cmd_SET_E_ROTATION_DISTANCE(self, gcmd):
        ...
        if invert_dir != orig_invert_dir:
            rotation_dist = -rotation_dist
        gcmd.respond_info("Extruder '%s' rotation distance set to %0.6f" % (self.name, rotation_dist))
...
class PrinterExtruder:
    ...
    def __init__(self, config, extruder_num):
        ...
        filament_diameter = config.getfloat('filament_diameter', minval=self.nozzle_diameter)
        self.filament_area = math.pi * (filament_diameter * 0.5) ** 2
        def_max_cross_section = 4.0 * self.nozzle_diameter ** 2
        def_max_extrude_ratio = def_max_cross_section / self.filament_area
        max_cross_section = config.getfloat('max_extrude_cross_section', def_max_cross_section, above=0.0)
        self.max_extrude_ratio = max_cross_section / self.filament_area
        logging.info('Extruder max_extrude_ratio=%.6f', self.max_extrude_ratio)
        toolhead = self.printer.lookup_object('toolhead')
        max_velocity, max_accel = toolhead.get_max_velocity()
        self.max_e_velocity = config.getfloat('max_extrude_only_velocity', max_velocity * def_max_extrude_ratio, above=0.0)
        self.max_e_accel = config.getfloat('max_extrude_only_accel', max_accel * def_max_extrude_ratio, above=0.0)
        self.max_e_dist = config.getfloat('max_extrude_only_distance', 50.0, minval=0.0)
        self.instant_corner_v = config.getfloat('instantaneous_corner_velocity', 1.0, minval=0.0)
        ffi_main, ffi_lib = chelper.get_ffi()
        ...
        if binding_ind_coil != None:
            self.binding_probe = self.printer.lookup_object('inductance_coil {}'.format(binding_ind_coil), None)
        ...
        self.check_interval = config.getfloat('check_interval', PARK_DETECTOR_LOOP_CHECK_INTERVAL, above=0.0)
        ...
        if xy_park_position is not None or xy_park_position_bak is not None:
            if xy_park_position_bak is not None:
                ...
                config.getfloat('y_idle_position', 50.0, minval=0.0)
            else:
                xy_park_position = config.getlists('xy_park_position', seps=(',', '\n'), count=2, parser=float)
                self.xy_park_position = list(xy_park_position[0])
                self.y_idle_position = config.getfloat('y_idle_position', 50.0, minval=0.0)
            self.grab_dir = config.getboolean('extruder_grab_dir', True)
            self.horizontal_move_x = config.getfloat('horizontal_move_x', 10.0, minval=0.0)
            self.retract_x_dist = config.getfloat('retract_x_dist', 1.5, minval=0.0)
            self.fast_move_speed = config.getfloat('fast_move_speed', 200.0, above=0.0)
            self.slow_move_speed = config.getfloat('slow_move_speed', 30.0, above=0.0)
            self.grab_speed = config.getfloat('grab_speed', 10.0, above=0.0)
            self.inser_buffer_dist = config.getfloat('insertion_buffer_dist', 5.0, minval=0.0)
        ...
        self.switch_accel = config.getfloat('switch_accel', 5000.0, above=0.0)
        ...
        gcode.register_mux_command('ACTIVATE_EXTRUDER', 'EXTRUDER', self.name, self.cmd_ACTIVATE_EXTRUDER, desc=self.cmd_ACTIVATE_EXTRUDER_help)
        gcode.register_mux_command('INNER_APPLY_FLOW_K', 'EXTRUDER', self.name, self.cmd_INNER_APPLY_FLOW_K)
        gcode.register_mux_command('SET_EXTRUDER_BASE_POSITION', 'EXTRUDER', self.name, self.cmd_SET_EXTRUDER_BASE_POSITION, desc='Set extruder base position')
        gcode.register_mux_command('SET_EXTRUDER_PARK_POSITION', 'EXTRUDER', self.name, self.cmd_SET_EXTRUDER_PARK_POSITION, desc='Set extruder park position')
        gcode.register_mux_command('MOVE_TO_PARK_CALIBRATION_POINT', 'EXTRUDER', self.name, self.cmd_MOVE_TO_PARK_CALIBRATION_POINT)
        gcode.register_mux_command('VERIFY_PARK_POSITION', 'EXTRUDER', self.name, self.cmd_VERIFY_PARK_POSITION)
    ...
    def active_binding_probe(self):
        if self.binding_probe is None:
            return
        logging.info('active new eddy current probe: {}'.format(self.binding_probe.name))
    ...
    def move(self, print_time, move):
        ...
        if axis_r > 0.0 and ((move.axes_d[0] or move.axes_d[1]) or self.is_calibrating_flow):
            can_pressure_advance = True
        self.trapq_append(self.trapq, print_time, move.accel_t, move.cruise_t, move.decel_t, move.start_pos[3], 0.0, 0.0, 1.0, can_pressure_advance, 0.0, start_v, cruise_v, accel, move.line)
    ...
    def analyze_switch_extruder_error(self, err_state=None):
        try:
            ...
            if len(activated) > 1:
                ...
                extruder_combinations = {(0, 1): 0, (0, 2): 1, (0, 3): 2, (1, 2): 3, (1, 3): 4, (2, 3): 5, (0, 1, 2): 6, (0, 1, 3): 7, (0, 2, 3): 8, (1, 2, 3): 9, (0, 1, 2, 3): 10}
            ...
            if unknown:
                ...
                unknown_combinations = {(0,): 0, (1,): 1, (2,): 2, (3,): 3, (0, 1): 4, (0, 2): 5, (0, 3): 6, (1, 2): 7, (1, 3): 8, (2, 3): 9, (0, 1, 2): 10, (0, 1, 3): 11, (0, 2, 3): 12, (1, 2, 3): 13, (0, 1, 2, 3): 14}
                unknown_index = unknown_combinations.get(tuple(unknown_sorted))
                if unknown_index is not None:
                    unknown_code = unknown_index + 1
            if error_messages:
                return ('; '.join(error_messages), activated, unknown, all_grip_states, activated_code, unknown_code)
            return None
        except Exception as e:
    ...
    def _handle_control_extruder_temp(self, web_request):
        try:
            temp = web_request.get_float('S', 0.0)
    ...
    def _cmd_SWITCH_EXTRUDER(self, gcmd, forced_park=False):
        ...
        if forced_park:
            action = 'PARK'
        try:
            if self.printer.lookup_object('homing_xyz_override', None) is not None or self.printer.lookup_object('safe_z_home', None) is not None:
                if not self.check_xy_homing():
                    gcode.run_script_from_command('G28 X Y')
            elif not self.check_homing():
            ...
            for i in range(10):
                ...
                if activate_status[0][1] != 2 or forced_skip:
                    ...
                    gcmd.respond_info('Retrying to get normal extruder activation status (attempt {}), forced_park: {}, retry_extruder_id: {}'.format(i, forced_park, retry_extruder_id))
                    toolhead.dwell(0.2)
                    toolhead.wait_moves()
            if activate_status[0][1] == 0 and activate_status[0][0] == self.name and (toolhead.get_extruder() is self) and (self.get_park_detector_status() is None and action is None or (self.get_park_detector_status() is not None and action != 'PARK')):
                gcmd.respond_info('Extruder %s already active' % (self.name,))
            ...
            saved_states = {'absolute_coord': gcode_move.absolute_coord, 'absolute_extrude': gcode_move.absolute_extrude, 'base_position': list(gcode_move.base_position), 'last_position': list(gcode_move.last_position), 'homing_position': list(gcode_move.homing_position), 'speed': gcode_move.speed, 'speed_factor': gcode_move.speed_factor, 'extrude_factor': gcode_move.extrude_factor, 'fan_speed': print_fan_speed, 'max_accel': toolhead.max_accel}
            if activate_status[0][1] == 0 or (forced_park and retry_extruder_id is not None):
                ...
                if cur_extruder.xy_park_position is not None and (activate_status[0][0] != self.name and action is None or (cur_extruder_state is None and action == 'PARK') or (cur_extruder_state is not None and (action == 'PARK' or (action == 'PICK' and activate_status[0][0] != self.name)))):
                    restore_state = True
                    if self.switch_accel != toolhead.max_accel:
                        toolhead.set_accel(self.switch_accel)
                    x_move_position = cur_extruder.xy_park_position[0] + [1, -1][not cur_extruder.grab_dir] * (cur_extruder.horizontal_move_x - cur_extruder.retract_x_dist)
            ...
            if self.xy_park_position is not None and (extruder_state is None or (extruder_state is not None and (not (activate_status[0][0] == self.name and activate_status[0][1] == 0)))):
                for i in range(10):
                    extruder_state = self.get_park_detector_status()
    ...
    def cmd_MOVE_TO_XY_IDLE_POSITION(self, gcmd):
        ...
        x_offset = gcmd.get_float('X_OFFSET', 0.0)
        y_offset = gcmd.get_float('Y_OFFSET', 0.0)
        z_offset = gcmd.get_float('Z_OFFSET', None)
        speed = gcmd.get_float('SPEED', 200.0)
        accel = gcmd.get_float('ACCEL', None)
        if not self.check_xy_homing():
            raise gcmd.error('Activate extruder must home XY first')
        x_idle_position = current_extruder.xy_park_position[0] + [1, -1][not current_extruder.grab_dir] * (current_extruder.horizontal_move_x - current_extruder.retract_x_dist)
    ...
    def cmd_SET_EXTRUDER_BASE_POSITION(self, gcmd):
        ...
        if not any_param_specified:
            if self.base_position is not None:
                gcmd.respond_info('Base position for %s: X:%.3f Y:%.3f Z:%.3f' % (self.name, self.base_position[0], self.base_position[1], self.base_position[2]))
            else:
        ...
        elif not self.update_extruder_config('base_position', new_base_position):
            gcmd.respond_info('Warning: Failed to save base position to config file')
        gcmd.respond_info('Base position for %s set to X:%.3f Y:%.3f Z:%.3f' % (self.name, new_base_position[0], new_base_position[1], new_base_position[2]))
    ...
    def cmd_SET_EXTRUDER_PARK_POSITION(self, gcmd):
        ...
        if y_idle is None:
            ...
            any_param_specified = True
        if not any_param_specified:
            gcmd.respond_info('Park position for %s: X:%.3f Y:%.3f Y_IDLE:%.3f' % (self.name, self.xy_park_position[0], self.xy_park_position[1], self.y_idle_position))
            return
        x_diff = abs(new_xy_park_position[0] - self.xy_park_position[0])
        y_diff = abs(new_xy_park_position[1] - self.xy_park_position[1])
        if (x_diff > MAX_ALLOWED_DIFFERENCE or y_diff > MAX_ALLOWED_DIFFERENCE) and (not force_save):
            diff_details = []
            ...
            msg = f"New park position differs too much. Exceeded: {', '.join(diff_details)}. Max allowed: {MAX_ALLOWED_DIFFERENCE:.3f}mm. Use FORCE_SAVE=1 to override."
            err_msg = '{"coded": "0003-0530-0000-0023", "msg":"%s"}' % msg
            raise gcmd.error(err_msg)
        gcmd.respond_info('Park position for %s changed from X:%.3f Y:%.3f Y_IDLE:%.3f to X:%.3f Y:%.3f Y_IDLE:%.3f' % (self.name, self.xy_park_position[0], self.xy_park_position[1], self.y_idle_position, new_xy_park_position[0], new_xy_park_position[1], y_idle))
        ...
        if extruder_bak is None or not os.path.exists(extruder_bak.config_path):
            ...
            park_data = {'xy_park_position': self.xy_park_position, 'y_idle_position': self.y_idle_position}
            if not self.update_extruder_config(None, park_data):
                gcmd.respond_info('Warning: Failed to save park position to config file')
        gcmd.respond_info('Park position for %s set to X:%.3f Y:%.3f Y_IDLE:%.3f' % (self.name, new_xy_park_position[0], new_xy_park_position[1], y_idle))
    ...
    def cmd_MOVE_TO_PARK_CALIBRATION_POINT(self, gcmd):
        ...
        if x_diff > MAX_ALLOWED_DIFFERENCE and (not force_move):
            msg = f'Move to park calibration position X differs too much from current position. Exceeded: X: {x_diff:.3f}mm. Max allowed: {MAX_ALLOWED_DIFFERENCE:.3f}mm.'
            err_msg = '{"coded": "0003-0530-0000-0023", "msg":"%s"}' % msg
            raise gcmd.error(err_msg)
        toolhead = self.printer.lookup_object('toolhead')
        pos = toolhead.get_position()
    ...
    def cmd_VERIFY_PARK_POSITION(self, gcmd):
        ...
        if x_diff > MAX_ALLOWED_DIFFERENCE and (not force_move):
            msg = f'Verify park position differs too much from current position. Exceeded: X: {x_diff:.3f}mm. Max allowed: {MAX_ALLOWED_DIFFERENCE:.3f}mm.'
            err_msg = '{"coded": "0003-0530-0000-0023", "msg":"%s"}' % msg
            raise gcmd.error(err_msg)
        try:
    ...
    for i in range(99):
        ...
        if 0 == i:
            ...
            if park_check_config is None and pe.park_detector is not None or (park_check_config is not None and pe.park_detector is None):
                raise config.error('park_detector config mismatch,  extruder: {},  {}: {} !!!'.format(['None', 'config'][park_check_config is not None], pe.name, ['None', 'config'][pe.park_detector is not None]))
            if park_position_config is None and pe.xy_park_position is not None or (park_position_config is not None and pe.xy_park_position is None):
                raise config.error('xy_park_position config mismatch, extruder: {},  {}: {} !!!'.format(['None', 'config'][park_position_config is not None], pe.name, ['None', 'config'][pe.xy_park_position is not None]))
